...
public class TodoController {
	...
	public ResponseEntity<?> createTodo(@AuthenticationPrincipal String userId, @RequestBody TodoDTO dto) {
		try {
	        ...
	        // 로그에 ResponseDTO 데이터 기록
			// @Data 어노테이션을 사용했기 때문에, Lombok은 toString() 메서드를 자동으로 생성
	        log.info("ResponseDTO: {}", response); //ResponseDTO: ResponseDTO(error=null, data=[TodoDTO(id=2c9..., title=테스트1, done=false)])
